...
def check(direct):
    try:
        ...
        xbmc.executebuiltin('Dialog.Show(busydialog)')
        dialog.ok('INVALID PATH', 'Kodi cannot write to the path you\'ve chosen. Please click OK', 'and choose another location. Currently network shares', 'are not supported so we suggest using something like a USB stick.')
        ADDON.openSettings()
